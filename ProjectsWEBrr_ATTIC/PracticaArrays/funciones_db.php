...
function conectarDB01(){
	...
	//$conexion = new mysqli('localhost','u199747581_rotrr','rotrr1983rugaral','u199747581_db01');
	//$conexion = new mysqli('localhost','root_ruben01','ruben01','db_backups');
...
function altaUsuario($nom,$ape,$nic,$correo,$pas){
	...
	$nivelAcceso = '5'; // =1, Super Admin(NEO). =2 User Admin (Merovingio). =3, User Avanzado(Oraculo). =4 User Medio(Morfeo), =5 User Basico(Trinity);
	$conex = conectarDB01();
	$resultconf = $conex->query("SET NAMES 'utf8'");
...
function reseteoPassword($correo,$nuevoPass){
	$conexion = conectarDB01();
	$resultconf = $conexion->query("SET NAMES 'utf8'");
	$consUsuario = $conexion->query("SELECT id_user FROM users WHERE email='$correo'; ");
