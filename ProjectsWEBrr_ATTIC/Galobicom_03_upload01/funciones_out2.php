...
switch($queFuncion){
	...
	case 7: $numJor= addslashes(strip_tags($_POST['jornada']));
			...
			registrarPartidoYgoleadoresRG($numJor,$idA,$idB,$golA,$golB,$fecHora,$arayIdJugadoresGol,$arayGolesJug);//insert en la "tabla resultados".
			break;
...
function guardarNoticiaRG($titulo,$contenido,$linkimg,$linkEnla){
	...
	$consNoti = $conex->query("SELECT count(idfk_user) as numNoti FROM noticiasga WHERE month(fec_publicado)='$mesAc	tual' AND idfk_user='$idUsuario'; ");
...
function guardarComentarioRG($mensaje,$idNoticia){
	...
	$conex = conectarDB01();
	$resultconf = $conex->query("SET NAMES 'utf8'");
	$consComent = $conex->query("SELECT count(idfk_user) as numComents FROM mensajesga WHERE month(fec_publicado)='$mesActual' AND idfk_user='$idUser'; ");
...
function registrarPartidoYgoleadoresRG($numJornada,$idEquipoA,$idEquipoB,$golA,$golB,$fecHora,$arayIdJug,$arayGol){  //Método para insertar o actualizar los registros de la
	...
	if($quiniela!='' || $quiniela==='x'){ //EL 0 Q ES UN POSIBLE VALOR REAL DE QUINIELA PHP LO INTERPRETA TB COMO NULL..O ESO PARECE.
		...
		//echo "<p>GolA:".$golA." GolB:".$golB." Quini: ".$quiniela."</p>"; //DEBUG CASERO
		/*for($cnt=0;$cnt<count($arayIdJug);$cnt++){
			echo "<p>".$arayIdJug[$cnt]."-".$arayGol[$cnt]."</p>";
		
		} */
		echo "<p>NumIdsJug: ".count($arayIdJug)."</p>";
		...
		//sleep(5);
		//*** AGREGAR LOS MÉTODOS PARA SUMAR LOS PUNTOS A LOS USUARIOS Q HAN REALIZADO LA QUINIELA.
	}     
}

function cargarCombosJugadoresGolRG($numJug){
	$con = conectarDB01();
...
<?php
		...
		cargarAlerts('warning','sm','No hay Goles (Porque sois unos paquetes,Ó todavía no se ha disputado el encuentro)');
	  }
	 }else{
	 	cargarAlerts('warning','sm','Error en Base de datos(db)');
	 }
